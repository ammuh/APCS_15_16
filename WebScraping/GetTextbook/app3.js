...
function scrapeSequence(){
    rp({ uri: 'http://iws.punahou.edu/user/DKiang/apcs', resolveWithFullResponse: true })
...
function pullL(){
    ...
    rp({ uri: url, resolveWithFullResponse: true, simple: false })
        .then(function(response) {
           if (response.statusCode == 200) {
...
function pullI(){
    ...
    .then(function(response) {
       if (response.statusCode == 200) {
            ...
            console.log('Intro from A'+global.lesson+ ' pulled.')
...
function pullV(){
    ...
    .then(function(response) {
       if (response.statusCode == 200) {
            ...
            $ = cheerio.load($('tr').html());
            logTextbook($('td[align = "left"]').html());
            console.log('Vocab from A'+global.lesson+ ' pulled.')
...
function pullS(){
    ...
    .then(function(response) {
       if (response.statusCode == 200) {
            ...
            $ = cheerio.load($('tr').html());
            logTextbook($('td[align = "left"]').html());
            console.log('Section '+ String.fromCharCode(global.section) +' from A'+global.lesson+ ' pulled.');
            global.section++;
            setTimeout(pullS(), 200);
        }
        else if(response.statusCode == 404){
...
function pullLab(){
    ...
    .then(function(response) {
       if (response.statusCode == 200) {
            ...
            $ = cheerio.load($('tr').html());
            logTextbook($('td[align = "left"]').html());
            console.log('Lab '+ global.lab +' from A'+global.lesson+ ' pulled.');
            global.lab++;
...
function pullSum(){
    ...
    .then(function(response) {
       if (response.statusCode == 200) {
            ...
            $ = cheerio.load($('tr').html());
            logTextbook($('td[align = "left"]').html());
            console.log('Summary from A'+global.lesson+ ' pulled.')
...
function iterA(){
    if(global.lesson < 24 && global.letter == 'A'){
        var footer = '<br><br><br>';
        logTextbook(footer);
        global.lesson++;
        global.section = 65;
